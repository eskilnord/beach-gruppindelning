...
export interface UseConfirmedAdvancedModeResult {
   ...
   * "Öppna avancerat läge" affordance outside the dedicated `UiModeSwitch` toggle (which already has
   * its own inline confirm). If the app is already in ADVANCED mode this is a same-mode no-op that
...
/**
 * v0.6.0 audit batch D (D2): a shared confirm gate for every "Öppna avancerat läge" affordance that
 * lives OUTSIDE the navbar's own `UiModeSwitch` (which already inlines the identical confirm modal on
 * its own toggle) - same copy (`sv.uiMode.enableConfirm`), same "you can always go back" framing, so
 * every entry point into ADVANCED mode asks the same deliberate-opt-in question rather than each
 * screen inventing its own variant (or, worse, skipping the confirm entirely - the bug this hook
 * fixes for PrioritiesPanel.tsx's "Öppna avancerat läge" button).
 */
