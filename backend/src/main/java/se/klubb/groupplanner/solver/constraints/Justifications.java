...
public final class Justifications {
    ...
    public record BlockDoubleBookedJustification(long blockId, long groupIdA, long groupIdB) implements ConstraintJustification {
    }

    /** 10.3 groupMaxSizeHard: a group exceeds its configured maxSize. */
    public record GroupOverMaxJustification(long groupId, int size, int maxSize) implements ConstraintJustification {
    }

    /** 10.9 timeAvailabilityHard: a player placed at a time slot they marked unavailable. */
    public record TimeUnavailableJustification(long participantId, long groupId, long timeSlotId, String label)
            implements ConstraintJustification {
    }

    /** 10.11/10.13 sameGroupHard/differentGroupHard: a MUST_SAME/MUST_DIFFERENT wish broken. */
    public record PairWishBrokenJustification(long wishFieldId, long aParticipantId, long bParticipantId, String type)
            implements ConstraintJustification {
    }

    /** 10.15 coachNoOverlap: one coach double-booked onto two overlapping group sessions. */
    public record CoachDoubleBookedJustification(long coachPersonId, long groupIdA, long groupIdB, String timeLabel)
            implements ConstraintJustification {
    }

    /** 10.16 playerNoOverlap: one person (via two PlayerAssignments) double-booked in-plan. */
    public record PlayerDoubleBookedJustification(long personId, long groupIdA, long groupIdB) implements ConstraintJustification {
    }

    /** 10.17 coachCannotTrainAndCoachSameTime: a person coaches and plays at an overlapping time. */
    public record TrainAndCoachClashJustification(long personId, long coachGroupId, long playGroupId, String timeLabel)
            implements ConstraintJustification {
    }

    /** 10.18 coachAvailabilityHard: a coach assigned to a group session they are unavailable for. */
    public record CoachUnavailableJustification(long coachPersonId, long groupId, long timeSlotId) implements ConstraintJustification {
    }

    /** 10.19 coachRequirementHard: a required coach slot left unfilled. */
    public record MissingCoachJustification(long groupId, int slotIndex) implements ConstraintJustification {
    }

    /** coachMaxGroups: a coach assigned to more groups than their configured cap. */
    public record CoachOverloadedJustification(long coachPersonId, int groups, int maxGroups) implements ConstraintJustification {
    }
    ...
    public record CoachWishJustification(long participantId, long coachPersonId, String type, boolean fulfilled)
            implements ConstraintJustification {
    }
    ...
    public record SavedPlanClashJustification(long groupId, long resourceId, String sourcePlanName, String timeLabel, String kind)
            implements ConstraintJustification {
    }

    /** unassignedPlayer: a player left on the waitlist (medium-level penalty, §2.1). */
    public record UnassignedPlayerJustification(long participantId, String displayName, int priority)
            implements ConstraintJustification {
    }
    ...
    public record GroupSizeDeviationJustification(long groupId, int size, int targetSize) implements ConstraintJustification {
    }

    /** 10.5 groupMinSizeSoft / groupMinSizeEmpty: a group is under minSize. */
    public record GroupUnderMinJustification(long groupId, int size, int minSize) implements ConstraintJustification {
    }

    /** 10.6 levelBalance: a group's level spread (LevelMath.sadPoints), for display parity with the
     * constraint's own matchWeight. */
    public record LevelSpreadJustification(long groupId, int spreadPoints, long meanScaled) implements ConstraintJustification {
    }
    ...
    public record GroupOrderInversionJustification(long higherGroupId, long lowerGroupId, int meanDiffPoints)
            implements ConstraintJustification {
    }

    /** 10.8 previousGroupContinuity: distance (in groupOrder steps) between a player's previous and
     * newly placed group. */
    public record ContinuityJustification(long participantId, int previousGroupOrder, int newGroupOrder)
            implements ConstraintJustification {
    }

    /** 10.10 timePreferenceSoft: a player placed on an allowed-but-not-preferred time. */
    public record TimePreferenceMissedJustification(long participantId, long groupId, long timeSlotId)
            implements ConstraintJustification {
    }

    /** 10.12/10.14 sameGroupSoft/differentGroupSoft: a WANT_SAME/WANT_DIFFERENT wish not honored. */
    public record PairWishSoftJustification(long wishFieldId, long aParticipantId, long bParticipantId, String type)
            implements ConstraintJustification {
    }

    /** 10.20 coachLevelFit: a coach's [canCoachMin, canCoachMax] band vs. the group's actual mean
     * level (both scaled x100). */
    public record CoachLevelMismatchJustification(long coachPersonId, long groupId, long groupMeanScaled, int canMinScaled, int canMaxScaled)
            implements ConstraintJustification {
    }
    ...
    public record LateTimeJustification(long groupId, int groupOrder, String timeLabel, String direction)
            implements ConstraintJustification {
    }
